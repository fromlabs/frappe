...
class KeypadFlut {
  ...
  KeypadFlut({required EventStream<NumericKey> keypadStream}) {
    ...
    final updateValueStream =
      ...
      return value10 <= 100000 ? value10 + NumericKey.values.indexOf(key) : null;
    });
...
abstract class FrappeBloc implements Disposable {
  ...
  FrappeBloc() { ReactiveScope.current.runTransaction(init); }
  void init();
  EventStream<E> registerEventStream<E>(EventStream<E> es) {
    _references.add(es.toReference()); return es;
  }
  ValueState<V> registerValueState<V>(ValueState<V> vs) {
    _references.add(vs.toReference()); return vs;
  }
  @override
  void dispose() { for (final ref in _references) { ref.dispose(); } }
}
